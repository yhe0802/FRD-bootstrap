...
frdsimu <- function(model.id, c, rho, cluster = NULL, 
  ...
  eachdraw <- function() {
    ...
    # for clustered data
    if (!is.null(cluster)) {
      bws <- rdbwselect(dta$y, dta$x, bwselect = "cerrd", cluster = dta$g, kernel = kernel)$bws
      ...
      if (heteroskedasticity == F) {
        bws <- rdbwselect(dta$y, dta$x, bwselect = "cerrd", kernel = kernel)$bws
        hIK <- IKbandwidth(dta$x, dta$y, kernel = kernel2)
      } else {
        bws <- rdbwselect(dta$y0, dta$x, bwselect = "cerrd", kernel = kernel)$bws
          ...
          for (con in conti) {
            for (k in kernels) {
              ...
              all.name <- paste(k, "_jump", j*100, rho.type,
                                cl.type, sd.type, t.type, "_v2", sep = "")
